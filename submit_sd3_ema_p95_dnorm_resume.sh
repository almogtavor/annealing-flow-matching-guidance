#!/bin/bash
#SBATCH --job-name=sd3-ema-p95-dnorm-resume
#SBATCH --nodes=1
...
#SBATCH --output=logs/ddp/slurm_sd3_ema_p95_dnorm_resume_%j.log
#SBATCH --error=logs/ddp/slurm_sd3_ema_p95_dnorm_resume_%j.log
...
CONFIG="${TRAIN_CONFIG:-scripts/config_sd3_ema_p95_dnorm_steps20.yaml}"
# Resume from latest checkpoint of job 247614
export ANNEALING_GUIDANCE_RESUME_FROM="${ANNEALING_GUIDANCE_RESUME_FROM:-$PROJECT_DIR/output/checkpoints_20260414_183012/checkpoint_step_11200.pt}"
echo "Starting SD3 DDP training with $NPROC GPUs (master_port=$MASTER_PORT)..."
echo "Config: $CONFIG"
echo "Resume from: $ANNEALING_GUIDANCE_RESUME_FROM"
